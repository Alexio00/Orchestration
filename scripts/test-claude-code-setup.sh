...
grep -Fxq '<!-- GENERAL-5-CLOUD-ADAPTER:BEGIN version=1.2.4 -->' "$fresh_policy"
...
grep -Fxq '<!-- GENERAL-5-CLOUD-ADAPTER:BEGIN version=1.2.4 -->' "$older_policy"
older_prefix_line="$(grep -nFx 'CUSTOM-PREFIX' "$older_policy" | cut -d: -f1)"
older_begin_line="$(grep -nFx '<!-- GENERAL-5-CLOUD-ADAPTER:BEGIN version=1.2.4 -->' "$older_policy" | cut -d: -f1)"
