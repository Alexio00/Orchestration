...
allowed_repositories="${GENERAL5_ALLOWED_REPOSITORIES:-}"
...
if [[ -n "$allowed_repositories" && ! "$allowed_repositories" =~ ^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+(,[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+)*$ ]]; then
  printf 'GENERAL5_ALLOWED_REPOSITORIES must be a comma-separated owner/repository allowlist.\n' >&2
  exit 1
fi
...
printf 'Разрешённые репозитории автоматической записи: %s\n\n' "${allowed_repositories:-не заданы}" >> "$tmp_policy"
